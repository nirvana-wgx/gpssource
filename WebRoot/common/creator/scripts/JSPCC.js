...
var JSPCC_default = [1, 0, 0];

function JSPCC_Setup()
{
 for(i=0;i<s.length-1;i++)
  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
 change(0);
 
 var Selected = fQueryString("selected", true);
 
 if(Selected=="")
 {
  var SelectedAr = JSPCC_default;
  
  for(var i=0, j=SelectedAr.length; i<j; i++)
  {
   var Sle = document.getElementById(s[i]);
   
   if(i===0)
   {
    Sle.selectedIndex = parseInt(SelectedAr[i])+1;   
   }
   else
   {
    Sle.selectedIndex = parseInt(SelectedAr[i])+1; 
