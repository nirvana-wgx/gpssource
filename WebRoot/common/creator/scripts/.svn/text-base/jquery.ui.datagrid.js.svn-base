		...
		**/
		...
		var measureContainerSize = function(){
			...
			if( bodyTableH > middleH) {
				...
				vScrollbarSize = scrollSize;
				//设置列宽
				tableTotalWidth = totalTableBodyWidth();	
				//显示垂直滚动条
				//设置滚动条容器的状态
				vScrollbar.css({
					top: titleH + 'px',
...
**/		
		...
		**/
		...
		this.getTableData = function(){
			var result = new Array();
			if($HeaderCols[1].find("table")[0]){//表格有数据
				var fixedRow = $HeaderCols[1].find("tbody").children("tr");
				var dataRow = $BodyCols[1].find("tbody").children("tr");
				var i = 0;
				var len = dataRow.length;
				while(i<len){
					result.push($.extend(serializeTableRow(dataRow[i]), serializeTableRow(fixedRow[i])));
					i = i+1;
				}
				delete i;
			}
		...
		**/
		 ...
		 * 通过条件查询返回行数据数组
		 */
		this.getRowDataByCond = function(cond){
			var rows = filterData(cond), result=[];
			$.each(rows, function(i, row){
				result.push($Instance.getRowData(row.rowIndex));
			});
			return result;
		};
