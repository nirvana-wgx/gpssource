...
// 为了解决在一个pane内同时有多个子元素。通过控制子元素的显隐属性动态显示不同的内容。在sizeContent()中用到
		,	innerAdaptable:			false		// true=pane内部第一个可见子元素将撑满pane内部。
/**----------------end---------------------------*/
	...
	,	north: {
		...
		,	size:					"auto"
		,	default_size:			"auto" /**---------attribute added by xiaoqing.qiu----------*/
	...
	,	east: {
		...
		,	size:					200
		,	default_size:			200 /**---------attribute added by xiaoqing.qiu----------*/
	...
	,		unionPanel:				null /**---------attribute added by xiaoqing.qiu----------*/
		}
 ...
 */
	...
	var setPaneMinMaxSizes = function (pane) {
		...
		,	altPaneSpacing	= !$altPane ? 0 : options[ altSide[pane] ].spacing_open
		,	altPaneSize		= ((!$altPane || !$altPane.is(':visible')) ? 0 : (dir=="horz" ? $altPane.outerHeight() : $altPane.outerWidth()))
		...
		,	minSize			= s.minSize || 0
//		,	maxSize			= Math.min(s.maxSize || 9999, limitSize)/*---changed by xiaoqing.qiu--*/
		,	maxSize			= Math.min(o.maxSize || 9999, limitSize)
...
/**-----------------------Add by xiaoqing.qiu-----------------------------*/
				if(options['center'].unionPanel==pane){
					//在pane的resize bar上添加center pane的toggle button
					var co = options['center'];
					$Rs['center'] = $R;
					$CT = $Ts['center'] = $("<div></div>");
					$CT
						// if paneSelector is an ID, then create a matching ID for the resizer, eg: "#paneLeft" => "paneLeft-toggler"
						.attr("id", (co.paneSelector.substr(0,1)=="#" ? co.paneSelector.substr(1) + "-toggler" : ""))
						.css(c.togglers.cssReq) // add base/required styles
						.attr("title", ($Ps['center'].is(":visible") ? co.togglerTip_open : co.togglerTip_closed))
				//		.click(function(evt){ toggle('center'); evt.stopPropagation(); })
						.mouseover(function(evt){ evt.stopPropagation(); }) // prevent resizer event
						// ADD CLASSNAMES - eg: class="toggler toggler-west toggler-west-open"?????????????????????
						.addClass( tClass +" "+tClass+_state+" "+ tClass+"-center"+_pane +" "+ tClass+"-center"+_state +" "+ tClass+"-center"+_pane+_state )
					;
					if(pane=='south' || pane == 'east')	{
						$CT.insertBefore($T); // append SPAN to resizer DIV
					}else{
						$CT.insertAfter($T);
					}
					$CT.click(function(evt){ toggle2('center'); evt.stopPropagation(); });
					$T.click(function(evt){ toggle2(pane); evt.stopPropagation(); });
				}else{
					$T.click(function(evt){ toggle(pane); evt.stopPropagation(); });
				}
/**----------------------------------end----------------------*/
...
/**-------------------------------Add by xiaoqing.qiu-------------------------*/
	 ...
	 * @param String  pane   The pane being toggled, ie: north, south, east, or west
	*/
	var toggle2 = function(pane) {
		var s = state[pane];
		if(pane == 'center'){
			var direPane = options[pane].unionPanel;//
			var direState = state[direPane];
			if(s.isHidden || s.isClosed){//center处于关闭状态，direPane处于最大化状态，需要打开center,还原direPane
				sizePane(direPane, options[direPane].original_size);
				state[direPane].isMaximum = false;
				$Ps['center'].show();
				if(s.isHidden){
					state['center'].isHidden = false;
				}
				if(s.isClosed){
					state['center'].isClosed = false;
				}
				var tClass	= options['center'].togglerClass
					,	_pane	= "-center-"+direPane
					,	_open	= "-open"
					,	_sliding= "-sliding"
					,	_closed	= "-closed"
					;
					$Ts['center']
						.removeClass( tClass+_closed +" "+ tClass+"-center"+_closed +" "+ tClass+_pane+_closed )
						.addClass( tClass+_open +" "+ tClass+"-center"+_open +" "+ tClass+_pane+_open )
						.attr("title", options['center'].togglerTip_open) // may be blank
					;
			}else{//center处于打开状态；需要关闭center,最大化direPane
				if(direState.isClosed){//需要打开direPane
					open(direPane);
				}
				var centerDim = getElemDims($Ps['center']);
				var direDim = getElemDims($Ps[direPane]);
				var size=0;
				if(c[direPane].dir=='horz'){
					size = parseInt(direDim.outerHeight+centerDim.outerHeight);
				}else{
					size = parseInt(direDim.outerWidth+centerDim.outerWidth);
				}
				//最大化direPane
				sizePane(direPane, size);
				state[direPane].isMaximum = true;
				//需要关闭center
				$Ps['center'].hide();
				//修改center toggle状态
				state['center'].isClosed = true;
				state['center'].isHidden = true;
				var tClass	= options['center'].togglerClass
				,	_pane	= "-center-"+direPane
				,	_open	= "-open"
				,	_sliding= "-sliding"
				...
				;
				$Ts['center']
					.removeClass(tClass+_open +" "+ tClass+"-center"+_open +" "+ tClass+_pane+_open)
					.addClass(tClass+_closed +" "+ tClass+"-center"+_closed +" "+ tClass+_pane+_closed)
					.attr("title", options['center'].togglerTip_closed) // may be blank
				;
			}
		}else{//点击的是某个周边panel的toggle
			if(s.isHidden || s.isClosed){//pane处于关闭状态,center处于最大化状态;需要打开pane，还原center
				open(pane);
				sizePane(pane, options[pane].original_size);
				return;
			}else{//pane处于打开状态
				//关闭pane，打开center
				close(pane);
				if(state['center'].isClosed || state['center'].isHidden){//pane处于最大化状态，center处于关闭状态，需要改变center的状态,修改toggle btn
					state[pane].isMaximum = false;
					sizeContent('center');
					$Ps['center'].show();
					state['center'].isClosed = false;
					state['center'].isHidden = false;
...
/**---------------------------------end----------------------------------*/
	...
	//	if (o.maxSize > 0) s.maxSize = min(s.maxSize, o.maxSize);
		...
		size = max(size, max(s.minSize, o.minSize));
		size = min(size, o.maxSize > 0 ? min(s.maxSize, o.maxSize) : s.maxSize);
		s.size = size; // update state
...
/**------------------------------Added by xiaoqing.qiu-----------------*/
	/**
	* 选取pane内部的第一个可视子元素填充满整个pane区域
	*/
	var adaptInnerVisibleContent = function (panes) {
		if (!panes || panes == "all") panes = c.allPanes;
		$.each(panes.split(","), function(i, elem) {
				var 
					pane	= str(elem)
				,	innerAdaptable = options[pane].innerAdaptable
				;
			if(innerAdaptable){
				var 
				...
				;
				$E = $($P.children(":visible").not(ignore).first());
				if (height > 0)
					height = cssH($E, height);
				if (height < 1)
					$E.hide(); // no room for content!
				else
					$E.css({ height: height }).show();
			}
			...
			if($P){
				...
				if($C){
					$P.children().each(function() {
						if (this == $Cs[pane][0]) return; // Content elem - skip
						var $E = $(this);
					...
					if (height < 1 && width < 1)
						...
						$C.css({ height: height, width:width }).show();
						$C.trigger("resized");
					}
				}else if(innerAdaptable){
					var $E = $($P.children(":visible").not(ignore).first());
					if (height > 0)
						height = cssH($E, height);
					if(width > 0){
						width = cssW($E, width);
					...
					if (height < 1 && width < 1)
						...
						$E.css({ height: height, width:width }).show();
						$E.trigger("resized");
					}
				}
			}
...
/*	
	var sizeContent = function (panes) {
		...
		$.each(panes.split(","), function(i, elem) {
			if (!$Cs[elem]) return; // NO CONTENT - skip
			var 
				pane	= str(elem)
			,	ignore	= options[pane].contentIgnoreSelector
			,	$P		= $Ps[pane]
...
*/
	...
	var sizeHandles = function (panes, onInit) {
		...
		$.each(panes.split(","), function() {
			...
			,	$R		= $Rs[pane]
			,	$T		= $Ts[pane]
			,	$CT		= $Ts['center'] //toggle for center panel
			,	isShareBorder = $Rs[pane] == $Rs['center'];
			;
			...
			//		height:	max(1, cssH($R, paneLen)) // account for borders & padding
				...
				,	bottom:	cDims.bottom - getPaneSize("south", true)
				});
...
/**---------------------------原版.end----------------------*/
/**---------------------------Add by xiaoqing.qiu----------------------*/
			//增加center panel的toggle button定位
			if ($T) {
				if (togLen == 0 || (s.isSliding && o.hideTogglerOnSlide)) {
					$T.hide(); // always HIDE the toggler when 'sliding'
					return;
				...
				var padding = 2;
				if(isShareBorder){//与center共用resize bar
					togLen = togLen*2 + padding;
				}
				if (!(togLen > 0) || togLen == "100%" || togLen > paneLen) {
					togLen = paneLen;
					offset = 0;
				...
				if (dir == "horz") { // north/south
					var width = cssW($T, togLen);
					if(isShareBorder){
						width = cssW($T, (togLen-padding)/2);
						$CT.css({
							width:	max(0, width)  // account for borders & padding
						,	height:	max(1, cssH($T, spacing)) // ditto
						,	left:	$CT.index() < $T.index() ? offset : offset+width+padding// TODO: VERIFY that toggler  positions correctly for ALL values
						});
						$T.css({
							width:	max(0, width)  // account for borders & padding
						,	height:	max(1, cssH($T, spacing)) // ditto
						,	left:	$CT.index() < $T.index() ? offset+width+padding : offset// TODO: VERIFY that toggler  positions correctly for ALL values
						});
						if ($TC) // CENTER the toggler content SPAN
							$TC.css("marginLeft", Math.floor((width-$TC.outerWidth())/2)); // could be negative	
					}else{
					...
					var height = cssH($T, togLen);
					if(isShareBorder){
						height = cssH($T, (togLen-padding)/2);
						$CT.css({
							height:	max(0, height)  // account for borders & padding
						,	width:	max(1, cssW($T, spacing))
						,	top:	$CT.index() < $T.index() ? offset : offset+height+padding 
						});
						$T.css({
							height:	max(0, height)  // account for borders & padding
						,	width:	max(1, cssW($T, spacing))
						,	top:	$CT.index() < $T.index() ? offset+height+padding : offset 
						});
						if ($TC) // CENTER the toggler content SPAN
							$TC.css("marginTop", Math.floor((height-$TC.outerHeight())/2)); // could be negative
					}else{
						$T.css({
							height:	max(0, height)  // account for borders & padding
						,	width:	max(1, cssW($T, spacing))
						,	top:	offset // TODO: VERIFY that toggler  positions correctly for ALL values
						});
						if ($TC) // CENTER the toggler content SPAN
							$TC.css("marginTop", Math.floor((height-$TC.outerWidth())/2)); // could be negative
					}
				}
			}
	...
	var resizeAll = function () {
		...
		if (checkH || checkW){
			...
			$.each(["south","north","east","west"], function(i,pane) {
				s = state[pane];
				dir = c[pane].dir;
				var sizedContent = false;
				if (!s.isClosed && ((checkH && dir=="horz") || (checkW && dir=="vert"))) {
					setPaneMinMaxSizes(pane); // update pane-state
					// shrink pane if 'too big' to fit
					if (s.isMaximum && state['center'].isClosed || s.size > s.maxSize){
						sizePane(pane, s.maxSize);
						sizedContent = true;
					}
				}
				if(options[pane].innerAdaptable && !sizedContent){
					sizeContent(pane);
				}
			});
		}
